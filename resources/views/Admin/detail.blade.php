...
@section('content')
                ...
                <div class="row row-bordered g-0">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-start">
                            ...
                            <form action="/admin/terima/{{ $data->id }}" method="POST">
                                @csrf
                                <div class="col-sm-5 me-2">
                                    <button type="submit" class="btn btn-success">Diterima</button>
                                </div>
                            </form>
                            <form id="formTolak" action="/admin/tolak/{{ $data->id }}" method="POST">
                                @csrf
                                <div class="col-sm-5">
                                    <button type="submit" class="btn btn-danger"
                                        confirm="Apakah Aduan ini akan Ditolak">Ditolak</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        document.getElementById("formTolak").addEventListener("submit", function(event) {
            var confirmation = confirm("Apakah Aduan ini akan Ditolak?");
            if (!confirmation) {
